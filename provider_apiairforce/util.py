...
def __getattr__(name: str) -> Any:
    """懒加载 Adapter / ApiairforceAdapter，避免顶层 import 触发 core 全量加载。"""
    if name in {"Adapter", "ApiairforceAdapter"}:
        from .core.adaptercore import Adapter as _Adapter  # noqa: PLC0415

        return _Adapter
    raise AttributeError(
        "module 'src.platforms.apiairforce.util' has no attribute {!r}".format(name)
    )
